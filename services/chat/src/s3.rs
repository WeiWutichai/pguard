...
//! presigner over `reqwest`. This avoids pulling the heavy `aws-sdk-s3` tree; the workspace
//! lists `reqwest` + `sha2` + `hmac` precisely for "chat signed URLs".
//!
...
impl S3Client {
    ...
    pub async fn upload(
        ...
        if !resp.status().is_success() {
            ...
            return Err(AppError::Internal("attachment upload failed".to_string()));
        }
        Ok(())
    }
...
mod tests {
    ...
    fn presigned_url_has_required_sigv4_params() {
        ...
        assert!(url.starts_with("https://cdn.pguard.app/pguard/chat/abc/def.jpg?"));
        assert!(url.contains("X-Amz-Algorithm=AWS4-HMAC-SHA256"));
        assert!(url.contains("X-Amz-Expires=3600"));
        assert!(url.contains("X-Amz-SignedHeaders=host"));
        // Credential's slashes are encoded (%2F), so it must contain the encoded scope.
        assert!(
            url.contains("X-Amz-Credential=AKIAEXAMPLE%2F20260607%2Fus-east-1%2Fs3%2Faws4_request")
        );
        assert!(url.contains("X-Amz-Date=20260607T120000Z"));
    ...
    fn presign_is_deterministic_for_fixed_time() {
        let c = client();
        let now = Utc.with_ymd_and_hms(2026, 6, 7, 12, 0, 0).unwrap();
        let a = c.presign(&c.endpoint, "GET", "chat/x/y.png", 3600, now);
        let b = c.presign(&c.endpoint, "GET", "chat/x/y.png", 3600, now);
    ...
    fn presign_signs_for_the_target_host() {
        ...
        let c = client();
        let now = Utc.with_ymd_and_hms(2026, 6, 7, 12, 0, 0).unwrap();
        let get = c.presign(&c.public_endpoint, "GET", "chat/x/y.png", 3600, now);
        let put = c.presign(&c.endpoint, "PUT", "chat/x/y.png", 300, now);
    ...
    fn download_url_uses_public_endpoint() {
        let c = client();
        let url = c.download_url("chat/abc/def.mp4");
        assert!(url.starts_with("https://cdn.pguard.app/pguard/chat/abc/def.mp4?"));
        assert!(
