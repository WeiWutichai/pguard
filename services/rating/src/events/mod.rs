//! Event layer — the transactional-outbox **relay** (producer half). rating writes the
//! review row AND an outbox row atomically (see `repo`); this background task drains the
//! outbox: poll unpublished rows → publish each to NATS (subject = `EventEnvelope.event_type`)
//! → stamp `published_at`. notification consumes `pguard.events.rating.submitted`.
//!
...
//! at-least-once (consumers dedupe on `event_id`). Mirrors payment's relay; concrete
//! [`JetStreamPublisher`] (no `dyn`) so the slice needs no `async-trait`.
...
pub async fn run_relay(db: sqlx::PgPool, nats_url: String) {
    loop {
        let publisher = match JetStreamPublisher::connect(&nats_url).await {
            Ok(p) => {
                tracing::info!(stream = STREAM, "rating outbox relay connected to NATS");
                p
            }
            Err(e) => {
                tracing::warn!("rating outbox relay NATS connect failed: {e}; retrying");
        ...
        loop {
            match drain_once(&db, &publisher).await {
                ...
                Ok(n) => tracing::debug!(published = n, "rating outbox relay drained batch"),
                Err(e) => {
                    tracing::warn!("rating outbox relay drain failed: {e}; reconnecting");
                    tokio::time::sleep(POLL_INTERVAL).await;
...
/// marked published only AFTER its publish acks (at-least-once; consumers dedupe on `event_id`).
