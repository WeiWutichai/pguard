//! booking-client adapter — the cross-service read the review authz verifies against.
//!
//! The rating service NEVER trusts a client-supplied guard/customer/status for the review
//! decision (CLAUDE.md authz rules). Instead it MINTS a short-lived service-JWT
//! (`encode_service_jwt("rating", ...)`) and GETs booking's `/internal/bookings/{id}`,
//! whose `ServiceCaller` guard validates the token. The returned `{ customer_id, guard_id,
//! status }` is authoritative: the caller must be the booking's customer and the booking
//! must be `completed`.
//!
//! A trait ([`BookingReader`]) decouples the handlers from `reqwest` so router/authz tests
//! are hermetic (no live booking service). Mirrors payment's `booking_client`.
...
/// Port: read the authoritative booking for a review decision. Implemented by
...
pub trait BookingReader: Send + Sync {
    /// Fetch `{ id, customer_id, guard_id, status }` for `booking_id`, or an error
...
impl BookingReader for HttpBookingReader {
    async fn get_booking(&self, booking_id: Uuid) -> Result<InternalBooking, AppError> {
        ...
        let token =
            encode_service_jwt("rating", &self.service_encoding_key, self.service_ttl_secs)?;
