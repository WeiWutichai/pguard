...
//! presigner over `reqwest`. Ported VERBATIM from `services/profile/src/s3.rs` (the house pattern,
//! itself from booking/chat) for PAYMENT SLIP images; extracting a shared crate is a noted
//! follow-up, the duplication is deliberate. This avoids pulling the heavy `aws-sdk-s3` tree; the
...
//! a short-lived (1h) signed GET URL. Payment slips are highly sensitive (PDPA — they carry the
//! customer's bank/PromptPay details) — the bucket is PRIVATE and reachable only via these
//! short-lived signed URLs, exactly like the guard ID documents.
//!
...
/// Presigned download URL lifetime — 1 hour (CLAUDE.md / spec). Retained from the ported house
/// pattern (with `download_url`/`public_endpoint`) for an admin slip re-read surface; the customer
/// pay flow only UPLOADS (it never echoes a slip URL back), so these are not yet on a request path.
#[allow(dead_code)]
...
pub struct S3Client {
    ...
    /// `endpoint` when unset (single-host dev). Only read by `download_url` (the admin re-read
    /// surface), which the customer pay flow does not call yet — see `DOWNLOAD_TTL_SECS`.
    #[allow(dead_code)]
...
impl S3Client {
    ...
    pub async fn upload(
        ...
        let resp = self
            ...
            .map_err(|e| {
                ...
                AppError::Internal("document upload failed".to_string())
            })?;
        if !resp.status().is_success() {
            let status = resp.status();
            let body = resp.text().await.unwrap_or_default();
            tracing::warn!("S3 upload returned {status}: {body}");
            return Err(AppError::Internal("document upload failed".to_string()));
        }
        Ok(())
    }

    /// Best-effort delete of `key` — compensation when the post-upload DB write fails, so a
    /// rejected upload doesn't accumulate orphaned objects. Errors are logged, never returned:
    /// an orphaned object is acceptable; failing the caller's (already-failed) request
    /// differently is not.
    ...
    /// Retained from the ported house pattern for an admin slip re-read; the customer pay flow
    /// stores the slip privately and never echoes a URL, so this is not yet on a request path.
    #[allow(dead_code)]
...
mod tests {
    ...
    fn presigned_url_has_required_sigv4_params() {
        ...
        let url = c.presign(
            &c.public_endpoint,
            "GET",
            "profile/abc/documents/def.jpg",
            DOWNLOAD_TTL_SECS,
            now,
        );

        assert!(url.starts_with("https://cdn.pguard.app/pguard/profile/abc/documents/def.jpg?"));
    ...
    fn presign_is_deterministic_for_fixed_time() {
        let c = client();
        let now = Utc.with_ymd_and_hms(2026, 6, 10, 12, 0, 0).unwrap();
        let a = c.presign(&c.endpoint, "GET", "profile/x/documents/y.png", 3600, now);
        let b = c.presign(&c.endpoint, "GET", "profile/x/documents/y.png", 3600, now);
    ...
    fn presign_signs_for_the_target_host() {
        ...
        let get = c.presign(
            &c.public_endpoint,
            "GET",
            "profile/x/documents/y.png",
            3600,
            now,
        );
        let put = c.presign(&c.endpoint, "PUT", "profile/x/documents/y.png", 300, now);
    ...
    fn download_url_uses_public_endpoint() {
        let c = client();
        let url = c.download_url("profile/abc/documents/def.jpg");
        assert!(url.starts_with("https://cdn.pguard.app/pguard/profile/abc/documents/def.jpg?"));
        assert!(
