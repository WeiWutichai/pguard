//! Event CONSUMER — projects `pguard.events.booking.*` into the `profile.guard_assignments`
//! IDOR read-model so profile can answer "does customer X have an active booking with guard Y?"
//! WITHOUT a cross-service FK or a synchronous cross-schema read (CLAUDE.md Data rules). This is
//! the gate the customer-readable guard mini-profile (`GET /guards/{id}/public`) relies on — a
//! customer may read a guard's name only while assigned to their active booking.
//!
//! It mirrors presence's `presence-booking-links` consumer (each service owns its OWN read-model
//! — they never share a table or a durable name).
//!
//! Resilience + correctness:
//!  - A durable pull consumer filtered to `pguard.events.booking.>` (JetStream at-least-once),
//!    durable name `profile-booking-links` (distinct from presence/notification/payment).
...
/// MUST differ from presence's `presence-booking-links` — two services sharing a durable name on
/// the same stream interfere (they'd split the message set between them).
const DURABLE: &str = "profile-booking-links";
...
///
/// SECURITY COUPLING: if a future booking event MOVES a job to a different guard (e.g. a
/// hypothetical `booking.reassigned`), it MUST be handled here AND in presence's identical
/// `active_for` — otherwise the OLD (customer, guard) link stays `active` and the customer could
/// keep reading the previous guard's name/location. The two read-models must not drift.
...
/// transient NATS outage must not silently kill the IDOR read-model's updates (mirrors the
/// outbox relay). Spawned as a background task by `main`.
pub async fn run_consumer(db: sqlx::PgPool, nats_url: &str) {
    loop {
        match connect_and_consume(&db, nats_url).await {
            Ok(()) => tracing::warn!("profile booking-links consumer stream ended; reconnecting"),
            Err(e) => tracing::warn!("profile booking-links consumer error: {e}; reconnecting"),
        }
...
async fn connect_and_consume(db: &sqlx::PgPool, nats_url: &str) -> Result<(), AppError> {
    ...
    tracing::info!(
        stream = STREAM,
        filter = FILTER,
        "profile booking-links consumer started"
    );
    ...
    while let Some(item) = messages.next().await {
        ...
        // poison the assignment read-model and unlock a stranger's guard name) is dropped,
...
/// producer's trace (booking→NATS→profile is one trace in Tempo).
async fn process(
    db: &sqlx::PgPool,
