...
//! presigner over `reqwest`. Ported from `services/booking/src/s3.rs` (itself from chat's house
//! pattern) for GUARD DOCUMENT images; extracting a shared crate is a noted follow-up,
//! duplication is deliberate per the PHASE spec §B4. This avoids pulling the heavy `aws-sdk-s3`
//! tree; the workspace lists `reqwest` + `sha2` + `hmac` precisely for these signed URLs.
//!
...
//! a short-lived (1h) signed GET URL. Guard ID documents are highly sensitive (PDPA) — the bucket
//! is PRIVATE and reachable only via these short-lived signed URLs.
//!
