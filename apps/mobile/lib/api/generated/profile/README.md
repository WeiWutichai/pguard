...
license, training cert, criminal check, driver license, passbook photo). Document image
UPLOAD (S3) is still a deferred follow-up, but document **expiry dates** can now be
captured (metadata only) via `document_expiries` on the guard profile submit
(`POST /profile/guard`) — these feed the admin \"expiring documents\" surface.
