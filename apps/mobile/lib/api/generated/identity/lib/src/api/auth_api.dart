...
class AuthApi {
  ...
  /// Validates the presented refresh token (from the body or the &#x60;refresh_token&#x60; cookie). On success it rotates: the presented token is revoked and a new access+refresh pair is issued in the SAME family. **Reuse detection:** presenting a token that was already rotated while its family is still live revokes the entire family and returns a generic 401. Expired or unknown tokens also return a generic 401.  **&#x60;SESSION_SUPERSEDED&#x60; (single-device sessions):** when the presented token&#39;s WHOLE family is revoked — this device was kicked by a newer guard/customer login, logged out, or force-revoked — the 401 carries the machine-readable &#x60;error.code &#x3D; SESSION_SUPERSEDED&#x60; so the app can show \&quot;บัญชีนี้ถูกเข้าสู่ระบบจากอุปกรณ์อื่น\&quot; (signed in from another device) and route to login instead of a generic session-expired. 
  ///
  /// Parameters:
  /// * [refreshRequest] 
  /// * [cancelToken] - A [CancelToken] that can be used to cancel the operation
