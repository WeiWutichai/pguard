...
class BookingsApi {
  ...
  /// A guard accepts → status &#x60;accepted&#x60;, &#x60;guard_id&#x60; set to the caller. Enqueues &#x60;pguard.events.booking.job_accepted&#x60; &#x60;{ booking_id, customer_id, guard_id }&#x60; into the outbox in the SAME transaction (transactional outbox). Rejected with 409 if the booking is not in a state from which acceptance is legal.  DIRECTED OFFER (C3): if the booking was directed at ONE guard (&#x60;target_guard_id&#x60; set), only that guard may accept — any other guard is rejected with **403 &#x60;NOT_OFFERED_TO_YOU&#x60;** (the app localizes the code). An OPEN booking (&#x60;target_guard_id&#x60; null) stays first-come.  EXPIRY (ISSUE 1): an open request whose scheduled window has already ENDED (&#x60;now &gt; scheduled_at + hours&#x60;) is no longer acceptable — **409 &#x60;BOOKING_EXPIRED&#x60;** (the app localizes it, e.g. \&quot;งานนี้หมดเวลาแล้ว\&quot;). Such requests are also hidden from &#x60;GET /bookings/open&#x60; and cancelled by the background scheduler (reason &#x60;system_expired&#x60;, no cancellation fee), so this 409 only fires in the race between a guard&#39;s tap and the sweep. 
  ///
  /// Parameters:
  /// * [id] 
  ...
  /// booking owns discovery but none of the guard catalog (profile), reviews (rating), or live presence (presence). This lists the APPROVED guard catalog (read from profile&#39;s &#x60;/internal/guards&#x60; over a service-JWT), RESTRICTED to guards presence reports as AVAILABLE via &#x60;/internal/online-guards&#x60; — they switched \&quot;พร้อมรับงาน\&quot; ON and hold a live tracking session — and enriches each with the guard&#39;s live rating summary (read from rating&#39;s &#x60;POST /internal/guards/rating-summaries&#x60; in ONE batch call over all surviving guard ids, not a per-guard lookup).  The three independent upstream reads (profile catalog, the local busy-guard DB, and presence) run CONCURRENTLY, so their latencies overlap rather than add serially.  Availability filter: an approved guard who has not switched Online Status on is dropped, even while their app streams GPS for an active job (that is a job-tracking session, not consent to be offered new work).  FAIL-CLOSED on presence (this REVERSES an earlier fail-open): if the presence consult errors or times out, the endpoint returns &#x60;503&#x60; with &#x60;error.code &#x3D; PRESENCE_UNAVAILABLE&#x60;. It does NOT return the approved list unfiltered — that silently breaks the hard \&quot;a guard who is not online must not appear\&quot; rule and, since pguard is pre-pay at accept, routes a PAYING customer to a guard who is not there. It also does not return an empty list, which clients render as \&quot;nobody is available\&quot; — an equally confident, equally wrong claim. Clients branch on the code to show a retryable error.  Best-effort on ratings: if the batch rating lookup fails, every guard still appears with &#x60;average_rating: null&#x60; and &#x60;review_count: 0&#x60; (a guard with no visible reviews likewise).  Nearest-first (C2): when the MEETUP point &#x60;lat&#x60;/&#x60;lng&#x60; is supplied, the surviving guards are sorted ASCENDING by the straight-line distance from each guard&#39;s LIVE position (per presence) to the meetup, and every entry carries &#x60;distance_m&#x60;. Omit the coordinates and the order is the catalog order with &#x60;distance_m&#x60; omitted (backward compatible). Guards with no known live position sort last. 
  ///
  /// Parameters:
  ...
  /// The CUSTOMER (request owner) decides: &#x60;approve&#x60; → &#x60;completed&#x60; (emits &#x60;pguard.events.booking.completed&#x60; with &#x60;booked_hours&#x60; + &#x60;actual_seconds&#x60;, driving payment proration) or &#x60;reject&#x60; → back to &#x60;arrived&#x60; (the guard finishes). Customer/admin only; the booking must be &#x60;pending_completion&#x60;.  **NO EXTENSION (QA #25):** &#x60;reject&#x60; is allowed ONLY while the booked window is still open (&#x60;now &lt;&#x3D; scheduled_at + hours&#x60;; the exact boundary still passes). Once the window has closed, sending the guard back to work would EXTEND the original job, which the product forbids — more service means a NEW booking through the normal flow. A late &#x60;reject&#x60; is **409 &#x60;JOB_WINDOW_CLOSED&#x60;** (the app localizes the code: \&quot;งานนี้สิ้นสุดเวลาแล้ว — หากต้องการ ใช้บริการต่อ กรุณาสร้างงานใหม่\&quot;). There is NO admin bypass: the worked duration of a job closed after its window is capped at the scheduled end, so re-opening one past that point can only produce unpaid guard work. &#x60;approve&#x60; stays available at any time, and if the customer does nothing the background scheduler auto-completes the job 30 minutes past the later of the scheduled end and the completion request. 
  ///
  /// Parameters:
  /// * [id] 
