...
/// * [cancellationReason] - Why the booking ended early — the stable code supplied to cancel (`changed_plan`|`mistake`|`not_needed`|`other`) or decline (`emergency`|`sick`|`cannot_reach`|`other`); see `CancelBookingRequest` / `DeclineBookingRequest` for the labels. NEVER localized text: clients map the code to a label. Deliberately typed as a plain string, not an enum, so an unrecognized code degrades to \"ไม่ระบุ\"/\"Not specified\" instead of failing response decoding. null on active bookings, and on terminal rows created BEFORE the reason became mandatory (migration 0009). Two SYSTEM codes are in neither endpoint's vocabulary — only the background scheduler writes them: `system_expired` (an OPEN request whose window ended with no guard) and `system_not_started` (QA #25 — an on-site guard who never started, closed 30 min past the booked window; full refund, no fee).
