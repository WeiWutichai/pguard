...
class LocationsApi {
  ...
  /// Internal read for booking&#39;s discovery (&#x60;/available-guards&#x60;) — the guards who are currently OFFERABLE, each with their LATEST fix position. Membership is &#x60;available_for_work AND is_online AND&#x60; session liveness (see the rule in &#x60;info&#x60;):    * the guard DECLARED \&quot;พร้อมรับงาน\&quot; on their tracking session — the hard product rule     (\&quot;Guard ที่ยังไม่ได้เปิด Online Status ต้องไม่แสดงอยู่ในหน้าเลือก Guard\&quot;). Merely     holding a GPS socket, e.g. to let a customer watch an active job, does NOT qualify.   * that session has been heard from in the last 2 minutes, so rows stranded by a     presence crash/redeploy expire on their own.  Still deliberately NOT gated on GPS freshness: the mobile uplink is movement-gated, so a stationary available guard&#39;s last fix ages past the freshness window while its socket stays up, and a freshness gate here would drop a connected, willing guard from discovery (bug B). Session liveness keys on the last FRAME, which keep-alives advance; GPS freshness survives only as the green-dot &#x60;is_live&#x60; DISPLAY on the read DTOs.  Guarded by a **service-JWT** (&#x60;serviceAuth&#x60;, aud &#x60;pguard-internal&#x60;), never reachable from the public edge (the gateway blocks &#x60;/internal/&#x60;). booking uses membership for the availability filter AND the coordinates to sort the customer&#39;s list nearest-to-meetup (C2); an error from this read makes discovery FAIL CLOSED (&#x60;503 PRESENCE_UNAVAILABLE&#x60;), never fall back to an unfiltered list. Narrow projection — id + position only, none of the heading/speed/accuracy the admin &#x60;/locations&#x60; bulk read carries (least-privilege). 
  ///
  /// Parameters:
  /// * [cancelToken] - A [CancelToken] that can be used to cancel the operation
  /// * [headers] - Can be used to add additional headers to the request
