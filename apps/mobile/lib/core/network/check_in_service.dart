...
abstract class CheckInService {
  ...
  /// schedule slot onto it before calling.
  Future<void> submit({
...
///  - **network** (no status) → a bilingual, retry-safe message (the server guards against
///    orphaned objects, so re-submitting after a network blip is safe).
class ApiCheckInService implements CheckInService {
  ...
  Future<void> submit({
    ...
    if (mime == null || !mime.startsWith('image/')) {
      throw const ApiException(
        message:
            'รองรับเฉพาะรูปภาพ JPEG/PNG/WEBP / Photo must be JPEG, PNG or WEBP',
      );
    }
    ...
    if (data is! Map<String, dynamic>) {
      throw const ApiException(
        message: 'ส่งรายงานเช็คอินไม่สำเร็จ / Check-in failed',
      );
    }
  ...
  /// Map a transport-level [ApiException] to a friendly bilingual one. Duplicate-hour 409 is
  /// handled by the caller (absorbed) and never reaches here.
  ApiException _friendly(ApiException e) {
    final status = e.statusCode;
    if (status == 409) {
      // The non-duplicate 409 is "hour N opens once N-1 hours have elapsed" (or not started).
      return const ApiException(
        message:
            'ยังไม่ถึงเวลาเช็คอินรอบนี้ ลองใหม่อีกครั้งภายหลัง / It is not time for this check-in yet — try again shortly',
        statusCode: 409,
      );
    }
    if (status == 413) {
      return const ApiException(
        message:
            'รูปภาพใหญ่เกินไป ถ่ายใหม่อีกครั้ง / Photo is too large — please retake it',
        statusCode: 413,
      );
    }
    if (status == 403 || status == 404) {
      return ApiException(
        message:
            "เช็คอินงานนี้ไม่ได้ (ไม่ใช่ผู้รับงาน) / You can't check in for this job",
        statusCode: status,
      );
    }
    if (e.isNetwork) {
      return const ApiException(
        message:
            'เครือข่ายขัดข้อง ส่งใหม่ได้เลย / Network problem — you can submit again',
      );
    }
