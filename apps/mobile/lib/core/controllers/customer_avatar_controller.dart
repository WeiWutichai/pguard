...
/// spinner. Magic-byte MIME so the declared Content-Type always matches the bytes (same gate the
/// server enforces).
@riverpod
class CustomerAvatarController extends _$CustomerAvatarController {
  ...
  Future<String?> build() async {
    ...
    try {
      final data = await api.get('/profile/customer/$customerId/avatar');
  ...
  Future<String?> upload(String filePath) async {
    ...
    if (customerId == null) return isThai ? 'ยังไม่พร้อม' : 'Not ready';
    ...
    try {
      ...
      final data =
          await api.post('/profile/customer/$customerId/avatar', data: form);
      final url =
  ...
  /// null for anything that isn't JPEG/PNG/WEBP. (Same logic as the guard avatar upload; a shared
  /// image-upload util is a small follow-up.)
