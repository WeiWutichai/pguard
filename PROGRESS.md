...
- [~] Phase 2 — Push-based mobile (WS แทน polling) — **foundation + first slice เสร็จ** (Riverpod codegen · design-token theme · FlutterSecureStorage · Dio ApiClient→/v1 proactive-refresh · reconnecting WS Bearer-on-upgrade · auth phone→OTP→PIN/login · **live booking-status ขับด้วย WS push ไม่มี Timer.periodic**); เหลือ guard job flow · presence/chat · registration เมื่อ backend พร้อม (track แยก PR — base `feat/v2-scaffold-notification`)
