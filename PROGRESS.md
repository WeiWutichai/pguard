...
- [~] Phase 3 — Split booking (call → payment → rating → assignment) — **payment money path ปิด end-to-end แล้ว** (expected_total server-computed + booking.completed consumer prorate idempotent + gateway `/v1/payments` + e2e ผ่าน infra จริง); เหลือ call / rating / assignment
...
- [ ] **§2.5** เดินตาม phase order ต่อ (Flutter Riverpod · rating/calling/presence/chat · profile-documents S3 upload · payment follow-ups: pay-after-complete reconciliation · OTLP exporter)

**Verify (Definition of Done):** `cargo build --workspace` ✅ · `cargo clippy --workspace --all-targets -D warnings` ✅ · `cargo test --workspace` ✅ **273 passed/0 failed** (api-gateway 56 · payment 43 · otp 42 · shared 35 · profile 26 · booking 25 · identity 25 · notification 16 · shared-events 4 · observability 1) · `cargo fmt --all --check` ✅ · gated/e2e tests กับ **Postgres+NATS จริง**: payment (charge-idempotent · proration-refund · finalize-idempotent · **admin-after-event ไม่ double-refund** · **surplus-tip ไม่ refund**) + **e2e book→pay→complete→finalize→replay** + booking (outbox/IDOR · completed-event timing) ทั้งหมดผ่าน
