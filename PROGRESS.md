...
- [~] Phase 3 — Split booking (call → payment → rating → assignment) — **payment money path** (e2e) **+ rating** + **discovery wired** (booking `GET /available-guards` ← profile approved-catalog + rating summary ผ่าน service-JWT, live 3-service e2e ผ่าน) แยกออกมาแล้ว; เหลือ call / assignment
...
- [ ] **§2.5** เดินตาม phase order ต่อ (Flutter Riverpod · calling/presence/chat · profile-documents S3 upload · guard-catalog pagination · payment pay-after-complete reconciliation · OTLP exporter)

**Verify (Definition of Done):** `cargo build --workspace` ✅ · `cargo clippy --workspace --all-targets -D warnings` ✅ · `cargo test --workspace` ✅ **303 passed/0 failed** (api-gateway 57 · payment 43 · otp 42 · shared 35 · profile 30 · booking 27 · identity 25 · rating 23 · notification 16 · shared-events 4 · observability 1) · `cargo fmt --all --check` ✅ · gated/e2e tests กับ **Postgres+NATS+Redis จริง**: payment (idempotent-charge · proration-refund · finalize-idempotent · admin-after-event ไม่ double-refund · surplus-tip ไม่ refund · e2e book→pay→complete→finalize→replay) + booking (outbox/IDOR · completed-event timing · **available-guards merge best-effort**) + rating (one-per-assignment+event · hidden-drops-from-summary · authz) + profile (**internal catalog approved-only + reject-trio**) + **live 3-service discovery e2e** ทั้งหมดผ่าน
