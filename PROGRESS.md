...
- [~] Phase 3 — Split booking (call → payment → rating → assignment) — **payment money path** (e2e) **+ rating** (reviews + admin moderation + guard summary, authoritative owner+completed via booking `/internal`, `rating.submitted` outbox, public filter `is_visible`) แยกออกมาแล้ว; เหลือ call / assignment
...
- [ ] **§2.5** เดินตาม phase order ต่อ (Flutter Riverpod · calling/presence/chat · profile-documents S3 upload · wire booking→rating summary · payment pay-after-complete reconciliation · OTLP exporter)

**Verify (Definition of Done):** `cargo build --workspace` ✅ · `cargo clippy --workspace --all-targets -D warnings` ✅ · `cargo test --workspace` ✅ **296 passed/0 failed** (api-gateway 56 · payment 43 · otp 42 · shared 35 · profile 26 · booking 25 · identity 25 · rating 23 · notification 16 · shared-events 4 · observability 1) · `cargo fmt --all --check` ✅ · gated/e2e tests กับ **Postgres+NATS+Redis จริง**: payment (charge-idempotent · proration-refund · finalize-idempotent · admin-after-event ไม่ double-refund · surplus-tip ไม่ refund · e2e book→pay→complete→finalize→replay) + booking (outbox/IDOR · completed-event timing) + **rating (one-per-assignment+event · hidden-drops-from-summary · authz/visibility)** ทั้งหมดผ่าน
